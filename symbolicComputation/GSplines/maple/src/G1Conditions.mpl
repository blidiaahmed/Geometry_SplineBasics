...
ValueAtEdgeFunctions := proc(Valence, knotsLists, degrees, ControleGridShape) 
  ...
  return F1ValuesAtEdge, F2ValuesAtEdge;
...
VanishAtVertex := proc(F1ValuesAtEdge, F2ValuesAtEdge) 
  ...
  #
  Expression := subs([v = 0, u = 0], diff(F1ValuesAtEdge, u)); 
  sys := [op(sys), Expression[1]];
  Expression := subs([v = 0, u = 0], diff(F1ValuesAtEdge, v));
  sys := [op(sys), Expression[1]];

  #
  Expression := subs([v = 0, u = 1], diff(F1ValuesAtEdge, u)); 
  sys := [op(sys), Expression[1]];
  Expression := subs([v = 0, u = 1], diff(F1ValuesAtEdge, v));
  sys := [op(sys), Expression[1]];

  #
  Expression := subs([v = 0, u = 0], diff(F1ValuesAtEdge, [u,v])); 
  sys := [op(sys), Expression[1]];

  #
  Expression := subs([v = 0, u = 0], diff(F2ValuesAtEdge, [u,v])); 
  sys := [op(sys), Expression[1]];

  #
  Expression := subs([v = 0, u = 1], diff(F1ValuesAtEdge, [u,v])); 
  sys := [op(sys), Expression[1]];

  #
  Expression := subs([v = 1, u = 0], diff(F2ValuesAtEdge, [u,v])); 
  sys := [op(sys), Expression[1]];
  return sys;
end proc;
